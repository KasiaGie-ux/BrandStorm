"""Tool handler functions — one per agent tool.

Each handler: validate args -> execute -> update canvas -> return (result_dict, events).
Called by ToolExecutor.execute(). Never raises — caller catches all exceptions.
"""
...
async def handle_set_identity(
    session: Session, args: dict,
...
async def handle_set_fonts(
    ...
    font_data = {
        "heading": {"family": heading_font, "style": args.get("heading_style", "")},
        "body": {"family": body_font, "style": args.get("body_style", "")},
    }
...
async def handle_propose_names(
    session: Session, args: dict,
) -> tuple[dict, list[dict]]:
    ...
    return {"status": "success", "names": [n["name"] for n in validated]}, [
        {"type": "name_proposals", "names": validated, "auto_select_seconds": 8},
        {"type": "canvas_update", "canvas": session.canvas.snapshot()},
    ]
...
async def handle_voiceover(
    ...
    story_url = await storage.upload_image(
        session_id=session.id,
    ...
    return {"status": "success", "audio_url": story_url}, [
...
async def handle_finalize(
    ...
    try:
        zip_url = await storage.create_zip(session_id=session.id, asset_urls=asset_urls)
    except Exception as e:
